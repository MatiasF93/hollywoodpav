Public Class Listado_alquileresXgenero
    ...
    Private Sub Listado_alquileresXgenero_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Dim sql As String = ""
        sql = "SELECT  peliculas.nombre as nombre_alquiler,  genero.descripcion as nombre_genero  "
        sql &= " FROM genero INNER JOIN peliculas ON peliculas.id_genero = genero.id_genero "
        sql &= " WHERE (peliculas.fecha_alquiler IS NOT NULL) AND (genero.descripcion) LIKE '%" & Me.txt_nombre_genero.Text & "%'"
